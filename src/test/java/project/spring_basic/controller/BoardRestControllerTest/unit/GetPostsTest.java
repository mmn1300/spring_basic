...
@Tag("controller")
@Tag("controller-unit")
public class GetPostsTest extends BoardRestControllerUnitTestSupport {
    ...
    public void getPosts() throws Exception {
        ...
        for (int i=1; i<15; i++){
            PostInfo postInfo = new PostInfo(
                Long.valueOf(i),
                "tttttttt" + Integer.toString(i),
                "테스트용 임시 계정" + Integer.toString(i),
                Integer.toString(i),
                Integer.toString(i), 
        ...
        when(boardService.getPostsInfo(0)).thenReturn(postsDTO);

        String ResponseJson = objectMapper.writeValueAsString(apiResponse); // 객체 -> json 문자열


        // when & then
        mockMvc.perform(get("/board/posts?page=0"))
    ...
    public void getPostsWhenExceptionOccurs() throws Exception {
        // given
        when(boardService.getPostsInfo(0))
